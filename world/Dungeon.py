...
class Dungeon:
    ...
    def BeginShifting(self, shift_x, shift_y):
        ...
        self.next_room = Room(self.player)
         ...
         # Ensure self.next_room is properly initialized
        if self.next_room is None:
         self.next_room = Room(self.player)  # Create the next room properly
        ...
        def next():
            ...
            # Reset all skeletons in the new room to their default state (idle)
            if self.next_room is not None:
            
                for entity in self.next_room.entities:
                    entity.ChangeState("idle")

            # Ensure player's health remains unchanged
